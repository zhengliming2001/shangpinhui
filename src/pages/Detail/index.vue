      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="index">
                ...
                <dd
                  ...
                  :key="item2.id"
                  @click="changeCheck(item2,item.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="skuNum" @change="changeNum"/>
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      // [{}]防止服务器数据没回来的时候报错
      // 也可以载子组件中重新计算一个属性给与一个||{}
      // 也可以在子组件使用props接收时，使用对象写法，给与一个默认值
      return this.skuInfo.skuImageList || [{}];
    },
  },
  methods:{
    changeCheck(item2,spuSaleAttrValueList){
      // item2——当前点击的项
      // spuSaleAttrValueList——同类型选项的的列表
      // 遍历选项
      // 排他思想
    ...
    async addShopcart(){
      try{
          ...
          // 成功后跳转路由,
          // 路由传参——商品对象，和购买的数量
          // 因为传递的商品对象为一个复杂的对象，所以放在本地会话存储内，路由传参只传购买数量
