  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in address" :key="item.id">
        ...
        <p @click="changeAddress(address, item)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="item in orderInfo" :key="item.skuId">
          <li>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ total.pricesum }}.00</span>
      </div>
  ...
  export default {
    ...
    computed:{
      ...
      total(){
        let goodssum=0;
        let pricesum=0;
          if(this.$store.state.trade.orderInfo.detailArrayList){
              this.orderInfo.forEach((item)=>{
              goodssum+=item.skuNum
              pricesum+=item.skuNum*item.orderPrice
          })
          }
          return {goodssum,pricesum} 
      },
      // 交易代码
      traderNo(){
        return this.$store.state.trade.orderInfo.tradeNo
      }
    },
    methods:{
          // 改变默认地址
          ...
          async subOrder(){
            // 发送请求给服务器，提交数据
            let result=await this.$API.reqSubOrder(this.traderNo,{
              // 收货人姓名
              ...
              "orderComment": "xxx",
              // 购买的物品信息
              "orderDetailList": this.orderInfo
              })
              if(result.code===200){
                alert('提交成功,'+'订单号:'+result.data)
                // 提交成功跳转路由
                // 跳转路由并携带订单号
                this.$router.push({name:'pay',params:{orderId:result.data}})
              }else{
